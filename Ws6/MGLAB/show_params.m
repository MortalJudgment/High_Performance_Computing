...
s  =[sprintf(' Solver                                   ')];
if (solver_flag==C_VMG & cycle_flag==V_CYCLE)
    s=[s;sprintf('    V-Cycle                               ')];
elseif (solver_flag==C_VMG & cycle_flag==W_CYCLE)
    s=[s;sprintf('    W-Cycle                               ')];
elseif (solver_flag==C_PCG)
    s=[s;sprintf('    PCG                                   ')];
elseif (solver_flag==C_BICG_STAB)
    s=[s;sprintf('    Bi-CG(Stab)                           ')];
elseif (solver_flag==C_CGS)
    s=[s;sprintf('    CGS                                   ')];
elseif (solver_flag==C_GMRES)
    s=[s;sprintf('    GMRES (%3d)                           ',restart)];
elseif (solver_flag==C_FMG)
    s=[s;sprintf('    FMG                                   ')];
elseif (solver_flag==C_SOR)
    if SOR_omega == 1
...
elseif (precon_flag==BLOCK_JACOBI)
    ,[r;sprintf('    Block-Jacobi                          ')];
elseif (precon_flag==GAUSS_SEIDEL)
    ,[r;sprintf('    Gauss-Seidel                          ')];
...
elseif (coarse_solver_flag==C_PCG)
    m=[m;sprintf('    Prolongation = PCG                    ')];
elseif (coarse_solver_flag==C_BICG_STAB)
    m=[m;sprintf('    Prolongation = Bi-CG(Stab)            ')];
elseif (coarse_solver_flag==C_GMRES)
